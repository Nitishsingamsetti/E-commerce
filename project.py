...
mydb=mysql.connector.connect(host='localhost',username='root',password='Suresh@416',db='ecommy')
app.secret_key=b'j\x03Qp\x8b\x94\xd8'
@app.route('/')
def home():
...
def additem():
    if not session.get('email'):
        ...
        if request.method=='POST':
            ...
            cursor=mydb.cursor(buffered=True)
            cursor.execute('insert into items(item_id,item_name,description,quantity,price,image_name,added_by,category)  values(uuid_to_bin(uuid()),%s,%s,%s,%s,%s,%s,%s)',[item,description,quantity,price,filename,session.get('email'),category])            
            mydb.commit()
            cursor.close()
...
def update_item(itemid):
    if not session.get('email'):
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select bin_to_uuid(item_id),item_name,image_name,price,quantity,category from items where added_by=%s and item_id=uuid_to_bin(%s)',[session.get('email'),itemid])
        item_data=cursor.fetchone()
        cursor.close()
        if request.method=='POST':
...
def adminprofile_update():
    if not session.get('email'):
        ...
        if request.method=='POST':
            ...
            cursor=mydb.cursor(buffered=True)
            cursor.execute('update admincreate set username=%s,address=%s,dp_image=%s,ph_no=%s where email=%s',[username,address,image,ph_no,session.get('email')])
            mydb.commit()
            cursor.close()
            flash(f"{session.get('email')} profile updated successfully")
            return redirect(url_for('adminprofile_upadate'))
        if admin_data:
    ...
    try:
        ...
        if request.method=='POST':
            ...
            if uotp==regdata['otp']:
                ...
                cursor=mydb.cursor(buffered=True)
                cursor.execute('insert into usercreate(username,user_email,address,password,gender) values(%s,%s,%s,%s,%s)',[regdata['username'],regdata['email'],regdata['address'],regdata['password'],regdata['gender']])
                mydb.commit()
                cursor.close()
                flash(f"{regdata['email']} Registration successfully Done")
                return redirect(url_for('userlogin'))
...
def dashboard(category):
    cursor=mydb.cursor(buffered=True)
    cursor.execute('select item_id,item_name,description,price,quantity,image_name from items where category=%s',[category])
    cursor.close()
...
def addreview():
    if session.get('useremail'):
        if request.method=="POST":
            ...
            mydb.commit()
            cursor.close()
            flash('Review has given to {itemid}')
...
def viewcart():
    ...
    if session.get(session.get('useremail')):
        items=session.get(session.get('useremail')) 
...
def orders():
    if session.get('useremail'):
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select * from orders wjere user=%s',[session.get('useremail')])
...
def contactus():
    if request.method=="POST":
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('insert into contactus value(%s,%s,%s)',[name,email,message])
        mydb.commit()
        cursor.close()
